...
public class Bow_v2 : Weapon {
    ...
    [SerializeField] Text arrowTypeTxt;
    ...
    private Vector3 handPosition = new Vector3(0.75f, -0.7f, 1.6f);
    ...
    public override void PrimaryFunction()
    {
        ...
        {
            dockedArrow = Instantiate(arrowType[arrowIndex], stringBend.transform.position - transform.right * 0.1f, Quaternion.LookRotation(-transform.up));
    ...
    public override void SecondaryFunction()
    {
        if (dockedArrow != null)
        {
            ...
            if (arrowIndex < arrowType.Length - 1) arrowIndex++;
            else arrowIndex = 0;

            shotReleased = true;
        }

        arrowTypeTxt.text = arrowType[arrowIndex].name;
