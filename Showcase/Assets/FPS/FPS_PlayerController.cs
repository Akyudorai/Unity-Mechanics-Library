...
public class FPS_PlayerController : MonoBehaviour {
    ...
    [SerializeField] Handgun_v1 equipped;
    ...
    private void Start()
    {
        if (equipped != null)
        {
            equipped.transform.localPosition = equipped.handPosition;
        }
            ...
            if (Input.GetKeyDown(KeyCode.Mouse1))
            {
                if (equipped != null)
                {
                    ...
                    if (equipped.isAiming)                    
                        equipped.transform.localPosition = equipped.aimedPosition;
    ...
    private void Interact()
    {
        ...
        if (Input.GetKeyDown(KeyCode.E) && delay == 0)
        {
            ...
            if (Physics.Raycast(rayOrigin, cam.transform.forward, out hit, 7.0f))
            {
                // Declare Target;
                ...
                //// If target is a vehicle, interact with it.
                //if (targetHit.GetComponent<I_Vehicle>() != null)
                //{
                //    targetHit.GetComponent<I_Vehicle>().Interact(gameObject);
                //    // gameObject.SetActive(false); // WARNING : A full vehicle will still disable the player
                //}

                //// if target is a terminal, interact with it.
                //if (targetHit.GetComponent<Terminal>() != null)
                //{
                //    targetHit.GetComponent<Terminal>().Interact(gameObject);
                //}
