        ...
        if (!shotReleased)
        {
            ...
            if (Input.GetKeyUp(KeyCode.Mouse0))
            {
                if (dockedArrow != null)
                {
                    ...
                    dockedArrow.GetComponentInChildren<Rigidbody>().AddForce(-transform.up * (100.0f * drawPower), ForceMode.Impulse);
                    dockedArrow.GetComponentInChildren<Rigidbody>().constraints = RigidbodyConstraints.None;
                    dockedArrow.GetComponentInChildren<BoxCollider>().isTrigger = false;
                }
